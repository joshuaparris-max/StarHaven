...
import { loadGameState, runLocalCommand, startLocalGame } from "@/lib/local-game";
...
export default function Game() {
  ...
  const handleCommand = (command: string) => {
    ...
    try {
      const result: CommandResult = runLocalCommand(command);
  ...
  const handleNewGame = async () => {
    ...
    try {
      const nextGame = await startLocalGame(gameMode, gameMode === 'custom' ? caseCode : undefined);
      ...
      <div className="h-screen w-full flex items-center justify-center bg-background">
        <Card className="max-w-2xl w-full p-8 text-center space-y-6">
          ...
          <Button 
            ...
            disabled={isStarting || (gameMode === 'custom' && !caseCode.trim())}
          ...
          <div className="flex flex-col h-full">
            ...
            />
              <CommandInput
                ...
                disabled={isRunningCommand || gameState.gameOver}
